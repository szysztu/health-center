...
public class DoctorService {
    ...
    private final static String doctorToken = "DOC";

    public DoctorDTO createDoctor(DoctorCreateDTO doctorCreateDTO) {
        var doctor = doctorMapper.toDoctor(doctorCreateDTO);

        if (!doctorCreateDTO.getToken().isEmpty()) {
            if (doctorCreateDTO.getToken().equals(doctorToken)) {
                ...
                throw new InvalidDoctorTokenException("Wrong token");
            }
        }
