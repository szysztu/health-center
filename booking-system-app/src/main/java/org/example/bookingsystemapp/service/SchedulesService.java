...
public class SchedulesService {
    ...
    public void addDoctorSchedule(DoctorScheduleDTO doctorScheduleDTO) {
        ...
        List<DoctorSchedule> existingSchedules = scheduleRepository.findFreeSchedulesOfDoctorByDoctorId(doctorScheduleDTO.getDoctorId());

        List<DoctorSchedule> schedules = doctorScheduleDTO.getTermins().stream()
                .map(dto -> {
                    scheduleValidator.validateStartTime(dto.getStartTime());
                    ...
                    if (exists) {
                        throw new NotAvailableException("Doctor already has a schedule at " + dto.getStartTime() + " on " + dto.getDay());
                    }

                    DoctorSchedule schedule = schedulesMapper.toDoctorSchedule(dto);
    ...
    public DoctorScheduleSingleDTO updateSingleSchedule(DoctorScheduleSingleDTO doctorScheduleSingleDTO) {
        ...
        if (doctorScheduleSingleDTO.getStartTime() != null || doctorScheduleSingleDTO.getDay() != null) {
            ...
            scheduleValidator.validateStartTimeAndDay(doctorScheduleSingleDTO.getStartTime(), doctorScheduleSingleDTO.getDay(), schedule);
    ...
    public BookingDTO createBooking(CreateBookingDTO createBookingDTO) {
        ...
        DoctorSchedule schedule = scheduleRepository.findById(createBookingDTO.getScheduleId())
                .orElseThrow(() -> new NotFoundException("Schedule not found with id " + createBookingDTO.getScheduleId()));

        if (schedule.isBooked()) {
            throw new NotAvailableException("Termin on " + schedule.getDay() + " at " + schedule.getStartTime() + " is already taken");
        }
    ...
    public List<ScheduleCriteriaReturnDTO> getSchedulesByCriteria(ScheduleCriteriaReqDTO scheduleCriteriaReqDTO) {
        ...
        List<ScheduleCriteriaReturnTempDTO> schedules = scheduleRepository.findSchedulesByCriteria(
    ...
    public List<FreeSchedulesOfDoctorDTO> getFreeSchedulesOfDoctor(Long doctorId) {
        ...
        List<DoctorSchedule> doctorSchedulesList = scheduleRepository.findFreeSchedulesOfDoctorByDoctorId(doctor.getId());
